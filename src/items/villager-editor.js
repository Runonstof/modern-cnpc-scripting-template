...
const ID_MAX = 221;
const ID_XP = 222;
const ID_SPECIAL = 223;
const ID_MULT = 224;
const ID_APPLY = 230;
const ID_ADD = 231;
const ID_DELETE = 232;
const ID_RESTOCK = 233;
const ID_RESTOCK_ALL = 234;

const API = Java.type("noppes.npcs.api.NpcAPI").Instance();
const ItemStack = Java.type("net.minecraft.world.item.ItemStack");
const MerchantOffer = Java.type("net.minecraft.world.item.trading.MerchantOffer");
const Component = Java.type("net.minecraft.network.chat.Component");
const GuiComponentClicked = Java.type("noppes.npcs.api.function.gui.GuiComponentClicked");

const UUID_KEY = "villagerEditorUuid";
...
function offerFromForm(gui, previous) {
  ...
  var offer = new MerchantOffer(
    slotStack(gui, ID_COST_A),
    slotStack(gui, ID_COST_B),
    slotStack(gui, ID_RESULT),
    uses,
    maxUses,
    xp,
    multiplier,
    previous ? previous.m_45375_() : 0
  );
...
function openEditor(player, target) {
  ...
  scroll.setOnClick(
    new GuiComponentClicked(function (activeGui, activeScroll) {
      var index = -1;
...
export function customGuiScroll(e) {
