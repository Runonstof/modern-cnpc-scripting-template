const WallClimberNavigation = Java.type(
  'net.minecraft.world.entity.ai.navigation.WallClimberNavigation'
);
const Mob = Java.type('net.minecraft.world.entity.Mob');
const Vec3 = Java.type('net.minecraft.world.phys.Vec3');
const MoverType = Java.type('net.minecraft.world.entity.MoverType');

const SPIDER_MODEL = 'minecraft:spider';
const SPIDER_TEXTURE = 'minecraft:textures/entity/spider/spider.png';
const SPIDER_SIZE = 5;
const CLIMB_SPEED = 0.2;
const PRESS_SPEED = 0.12;
...
function setMobNavigation(mc, nav) {
  ...
  const field = Mob.class.getDeclaredField('f_21344_');
...
function horizontalToward(npc, target) {
  ...
  return { x: (dx / len) * PRESS_SPEED, z: (dz / len) * PRESS_SPEED };
}
...
function applyWallClimb(npc) {
  ...
  const press = horizontalToward(npc, target);
  // m_20242_ = net.minecraft.world.entity.Entity#setNoGravity
  mc.m_20242_(true);
