...
function startRefill(player) {
  ...
  var slots = syncOrbitSlots(player);
  var slot = firstEmptySlot(slots);
  if (slot < 0) {
    return false;
  }
  var dest = overheadPoint(player, BLOCK_SCALE);
  var uuid = spawnOrbitBlock(player.getWorld(), player.getX(), player.getY(), player.getZ(), player);
